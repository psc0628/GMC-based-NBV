...
public:
	...
	double movement_cost;//总的运动代价

	int sampling_method_of_viewspace = 0; //视点空间采样方法, 0 球面均匀， 1 随机
	int first_view_id = 0; //初始视点编号
	int cloud_points_number; //点云GT中的点数

	string save_path;
	string pre_path = "../GMC/";
	string vs_path = "GMC_vs_random/";

	Share_Data(string _config_file_path, string test_name = "", int _first_view_id = -1, double _visble_rate = -1, double _move_rate = -1, int _method_id = -1, double _resize_scale = -1, double _skip_threshold = -1)
	{
		process_cnt = -1;
		...
		fs["sampling_method_of_viewspace"] >> sampling_method_of_viewspace;
		...
		fs["outer_sphere_radius_rate"] >> outer_sphere_radius_rate;
		fs["inner_sphere_radius_rate"] >> inner_sphere_radius_rate;
		...
		if (_first_view_id >=0 ) first_view_id = _first_view_id;
		if (_visble_rate >=0 ) visble_rate = _visble_rate;
		if (_move_rate >=0 ) move_rate = _move_rate;
		if (_method_id >=0 ) method_of_IG = _method_id;
		if (_resize_scale >=0 ) resize_scale = _resize_scale;
		else {
			//没给值就读文件默认值
			ifstream fin_GT_size("../Ground_size/" + name_of_pcd + ".txt");
			if (fin_GT_size.is_open()) {
				fin_GT_size >> resize_scale;
			}
		}
		if (_skip_threshold >=0 ) skip_threshold = _skip_threshold;
		//读取转换后模型的pcd文件
		...
		//read viewspace
		ifstream fin_sphere("../pack.3.1592.txt");
		if (fin_sphere.is_open()) {
			pt_sphere.resize(1592);
			...
			pt_norm = pt0.norm();
			cout << "Sphere code views readed." << endl;
		}
		else {
			cout << "No sphere code. Random." << endl;
		}
		//采样方法不同保存路径不同
		if (sampling_method_of_viewspace == 0) {
			vs_path = "GMC_vs_random/";
		}
		else if (sampling_method_of_viewspace == 1) {
			vs_path = "GMC_vs_uniform/";
		}
		
		//Octomap
		if (method_of_IG == NBVNET) octomap_resolution = 0.015;
		...
		save_path = pre_path + vs_path + name_of_pcd + "_v" + to_string(first_view_id) + '_' + to_string(method_of_IG);
		if (method_of_IG == 0) {
			//save_path += "_true";
			//save_path += '_' + to_string(cost_weight);
			if (his_on == false) save_path += "_nohis";
			if (move_cost_on == true) save_path += '_' + to_string(move_weight);
		}
		if (method_of_IG == 10) {
			//save_path += '_' + to_string(visble_rate);
			//save_path += '_' + to_string(move_rate);
			//save_path += '_' + to_string(skip_threshold);
			//save_path += "_withF";
			//save_path += "_nogreedy";
		}
