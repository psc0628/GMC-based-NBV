...
public:
	...
	double outer_sphere_radius_rate;
	double inner_sphere_radius_rate;
	int first_view_id;
	int occupied_voxels;						
	double map_entropy;	
	bool object_changed;
	...
	bool vaild_view(View& view) {
		...
		bool vaild = true;
		//物体bbx扩大2倍内不生成视点
		if (x > object_center_world(0) - inner_sphere_radius_rate * predicted_size && x < object_center_world(0) + inner_sphere_radius_rate * predicted_size
		&&  y > object_center_world(1) - inner_sphere_radius_rate * predicted_size && y < object_center_world(1) + inner_sphere_radius_rate * predicted_size
		&&  z > object_center_world(2) - inner_sphere_radius_rate * predicted_size && z < object_center_world(2) + inner_sphere_radius_rate * predicted_size) vaild = false;
		//在半径为4倍BBX大小的球内
		if (pow2(x - object_center_world(0)) + pow2(y - object_center_world(1)) + pow2(z- object_center_world(2)) - pow2(outer_sphere_radius_rate * predicted_size) > 0 ) vaild = false;
		//八叉树索引中存在且hash表中没有
	...
	void get_view_space(vector<Eigen::Vector3d>& points) {
		...
		if (share_data->sampling_method_of_viewspace == 1) { //如果是均匀采样
			for (int i = 0; i < share_data->pt_sphere.size() && viewnum < num_of_views; i++) {
				//if (share_data->pt_sphere[i][2] < 0) continue;
				double scale = 1.0 / share_data->pt_norm * predicted_size * (outer_sphere_radius_rate + inner_sphere_radius_rate) / 2.0; //predicted_size * sqrt(3) * ?
			...
			cout << views.size() << " unifrom views getted." << endl;
		}
		
		/*
		//第一个视点固定为模型中心
		View view(Eigen::Vector3d(object_center_world(0) - predicted_size * ((outer_sphere_radius_rate + inner_sphere_radius_rate) / 2.0), 0, 0));
		...
		*/
		...
		while (viewnum != num_of_views) {
			//3倍BBX的一个采样区域
			double x = get_random_coordinate(object_center_world(0) - predicted_size * outer_sphere_radius_rate, object_center_world(0) + predicted_size * outer_sphere_radius_rate);
			double y = get_random_coordinate(object_center_world(1) - predicted_size * outer_sphere_radius_rate, object_center_world(1) + predicted_size * outer_sphere_radius_rate);
			double z = get_random_coordinate(object_center_world(2) - predicted_size * outer_sphere_radius_rate, object_center_world(2) + predicted_size * outer_sphere_radius_rate);
			...
			if (vaild_view(view)) {
				...
				//}
				//else view.robot_cost = (Eigen::Vector3d(now_camera_pose_world(0, 3), now_camera_pose_world(1, 3), now_camera_pose_world(2, 3)).eval() - view.init_pos).norm();
				views.push_back(view);
				views_key_set->insert(octo_model->coordToKey(x,y,z));
				viewnum++;
			}
			sample_num++;
			if (sample_num >= 10000 * num_of_views) {
	...
	View_Space(int _id, Share_Data* _share_data, Voxel_Information* _voxel_information, pcl::PointCloud<pcl::PointXYZRGB>::Ptr cloud, int _first_view_id) {
		...
		outer_sphere_radius_rate = share_data->outer_sphere_radius_rate;
		inner_sphere_radius_rate = share_data->inner_sphere_radius_rate;
		...
		//检测viewspace是否已经生成
		ifstream fin(share_data->pre_path +share_data->vs_path + "/viewspace/" + share_data->name_of_pcd + ".txt");
		if (fin.is_open()) { //存在文件就读视点集合
			...
			for (int i = 0; i < num_of_views; i++) {
				//if (share_data->move_cost_on) {
				if (i == first_view_id) views[i].robot_cost = 0.0;
				else {
			...
			share_data->access_directory(share_data->pre_path + share_data->vs_path + "/viewspace/");
			ofstream fout(share_data->pre_path + share_data->vs_path + "/viewspace/" + share_data->name_of_pcd + ".txt");
			fout << num_of_views << '\n';
	...
	void update(int _id, Share_Data* _share_data, pcl::PointCloud<pcl::PointXYZRGB>::Ptr cloud,pcl::PointCloud<pcl::PointXYZRGB>::Ptr update_cloud) {
		...
		for (int i = 0; i < views.size(); i++) {
			...
			//if (share_data->move_cost_on) {
				...
				views[i].robot_cost = local_path.second;
			//}
			//else views[i].robot_cost = (Eigen::Vector3d(now_camera_pose_world(0, 3), now_camera_pose_world(1, 3), now_camera_pose_world(2, 3)).eval() - views[i].init_pos).norm();
		}
		//插入点云至中间数据结构
		double now_time = clock();
		double map_size = predicted_size + 3.0 * octomap_resolution;
		...
		cout << "Cloud " << id << " voxels(rate/GT) " << 1.0 * voxel->size() / share_data->cloud_points_number << endl;
		ofstream fout_cloud(share_data->save_path + "/quantitative/Cloud" + to_string(id) + ".txt");
		fout_cloud << voxel->size() << '\t' << 1.0 * voxel->size() / share_data->cloud_points_number << endl;
		delete voxel;
	}
